...
vec_control::PurePursuit::PurePursuit() {
  ...
  // Node paramters
  nh_private.param<double>("ld_gain", ld_gain_, 5.0);
  nh_private.param<double>("min_ld", min_ld_, 5);
  ...
  nh_private.param<std::string>("base_frame", base_frame_, "ego_vehicle");
  ld_ = min_ld_;
  ...
  ros::Subscriber odom_sub_ =
      nh_.subscribe("/carla/ego_vehicle/odometry", 1, &PurePursuit::odom_clk_, this);
    ...
    if (got_path_) {
      ...
      try {
        base_location_ = tfBuffer_.lookupTransform(
            map_frame_, base_frame_, ros::Time(0), ros::Duration(0.5));
